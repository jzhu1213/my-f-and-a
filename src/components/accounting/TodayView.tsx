...
import type { Budget, Transaction, TransactionCategory, TransactionType } from '@/types'
import {
...
export function TodayView({
  ...
  onLogExpense, onLogIncome, onOpenLimits, onViewHistory, onEditTransaction,
}: TodayViewProps) {
  ...
  // Sort: over budget first, then lowest remaining, then categories with limits
  ...
  return (
    <div className="pb-24">
      ...
      <div className="px-6 pt-12 pb-6" style={{ borderBottom: '1px solid var(--border)' }}>
        ...
        {isLoading ? (
          <div className="mb-6">
            ...
            <p style={{ marginTop: '8px', fontSize: '13px', color: 'var(--sub)' }}>&nbsp;</p>
          </div>
        ) : noLimitsSet ? (
          <div className="mb-6">
            ...
            <button onClick={onOpenLimits} className="btn-primary w-full">
              Set weekly limits
            </button>
          </div>
        ) : (
          <div className="mb-6">
            <p style={{
              ...
              color: heroAmount?.over ? 'var(--red)' : 'var(--text)',
            }}>
      ...
      {/* ── Categories: can I spend? ────────────────────────────── */}
      <div className="px-6 pt-6">
        <p className="label mb-4">By category</p>
        <p style={{ fontSize: '13px', color: 'var(--muted)', marginBottom: '16px', marginTop: '-8px' }}>
          Tap a category to log spending there
        </p>

        {sortedRows.map(row => {
          const leftLabel = (() => {
            if (!row.hasLimit) {
              return row.weeklySpent > 0
                ? `$${row.weeklySpent.toFixed(0)} spent`
                : 'no limit'
            }
          ...
          return (
            <button
              key={row.category}
              onClick={() => onLogExpense(row.category)}
        ...
        <div className="px-6 pt-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              style={{ fontFamily: 'Space Mono, monospace', fontSize: '11px', color: 'var(--muted)' }}
              onMouseEnter={e => (e.currentTarget.style.color = 'var(--sub)')}
              onMouseLeave={e => (e.currentTarget.style.color = 'var(--muted)')}
            >
              all →
          ...
          {recentTxs.map(tx => {
            ...
            return (
              ...
              >
                ...
                <span style={{
                  fontFamily: 'Space Mono, monospace', fontSize: '14px', flexShrink: 0,
      ...
      <div className="px-6 pt-6">
        ...
        {showMonthSummary && (
          <div className="pb-4 animate-fade-in">
            {showIncomeNudge ? (
              <p style={{ fontSize: '14px', color: 'var(--sub)' }}>
                Log income to see your monthly balance
              </p>
            ) : (
              <div className="flex gap-8">
                <div>
                  <p style={{ fontFamily: 'Space Mono, monospace', fontSize: '18px', color: 'var(--green)' }}>
                    +${monthIncome.toLocaleString()}
                  </p>
                  <p className="label mt-1">income</p>
                </div>
                <div>
                  <p style={{ fontFamily: 'Space Mono, monospace', fontSize: '18px', color: 'var(--red)' }}>
                ...
                <div>
                  <p style={{
                    ...
                    color: monthIncome - monthExpenses < 0 ? 'var(--red)' : 'var(--text)',
                  }}>
